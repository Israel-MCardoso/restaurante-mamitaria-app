...
import { useCart, type CartAddonSelection } from '@/contexts/CartContext';

interface StorefrontAddon { id: string; name: string; price: number }
interface StorefrontProduct {
  id: string; name: string; description?: string | null; price: number; promo_price?: number | null; image_url?: string | null; addons?: StorefrontAddon[];
}
interface StorefrontCategory { id: string; name: string; products: StorefrontProduct[] }
interface Props {
...
export function StorefrontExperience({ restaurant, categories }: Props) {
  ...
  useEffect(() => { setRestaurantScope(restaurant.id); }, [restaurant.id, setRestaurantScope]);

  const cartItems = useMemo(() => items.filter((item) => !restaurantId || restaurantId === restaurant.id), [items, restaurant.id, restaurantId]);
  ...
  const selectedAddons = useMemo<CartAddonSelection[]>(() => {
    ...
    return selectedProduct.addons.map((addon) => {
      const quantity = selectedAddonQuantities[addon.id] ?? 0;
      if (quantity <= 0) return null;
      return { id: addon.id, addon_id: addon.id, name: addon.name, quantity, unitPrice: addon.price, totalPrice: addon.price * quantity };
    }).filter((addon): addon is CartAddonSelection => addon !== null);
  }, [selectedAddonQuantities, selectedProduct]);
  const configuredBasePrice = useMemo(() => selectedProduct ? (selectedProduct.promo_price ?? selectedProduct.price) : 0, [selectedProduct]);
  const configuredUnitPrice = useMemo(() => configuredBasePrice + selectedAddons.reduce((sum, addon) => sum + addon.totalPrice, 0), [configuredBasePrice, selectedAddons]);

  const openConfigurator = (product: StorefrontProduct) => { setSelectedProduct(product); setProductNotes(''); setSelectedAddonQuantities({}); };
  const closeConfigurator = () => { setSelectedProduct(null); setProductNotes(''); setSelectedAddonQuantities({}); };
  const changeAddonQuantity = (addonId: string, nextQuantity: number) => {
    setSelectedAddonQuantities((current) => {
      if (nextQuantity <= 0) {
        const next = { ...current }; delete next[addonId]; return next;
      }
  ...
  const addConfiguredProductToCart = () => {
    ...
    addItem({
      cartKey: buildCartKey(selectedProduct.id, notes, selectedAddons),
  ...
  return (
    <main className="page-shell pt-10 text-stone-900">
      ...
      {selectedProduct ? <ProductConfigurator product={selectedProduct} notes={productNotes} setNotes={setProductNotes} selectedAddons={selectedAddons} selectedAddonQuantities={selectedAddonQuantities} configuredBasePrice={configuredBasePrice} configuredUnitPrice={configuredUnitPrice} changeAddonQuantity={changeAddonQuantity} onClose={closeConfigurator} onConfirm={addConfiguredProductToCart} /> : null}
      {isCartOpen ? <CartDrawer items={cartItems} total={total} onClose={() => setIsCartOpen(false)} updateItemQuantity={updateItemQuantity} /> : null}
    </main>
  );
}

function ProductConfigurator({ product, notes, setNotes, selectedAddons, selectedAddonQuantities, configuredBasePrice, configuredUnitPrice, changeAddonQuantity, onClose, onConfirm }: {
  product: StorefrontProduct; notes: string; setNotes: (value: string) => void; selectedAddons: CartAddonSelection[]; selectedAddonQuantities: Record<string, number>;
...
function CartDrawer({ items, total, onClose, updateItemQuantity }: {
  items: Array<{ cartKey: string; id: string; name: string; price: number; quantity: number; imageUrl?: string | null; description?: string | null; notes?: string | null; addons?: CartAddonSelection[] }>;
  total: number; onClose: () => void; updateItemQuantity: (cartKey: string, quantity: number) => void;
}) {
  return (
            ...
            <div key={item.cartKey} className="soft-card rounded-[1.5rem] p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex gap-4">
                  <div className="h-20 w-20 overflow-hidden rounded-2xl bg-stone-100">{item.imageUrl ? <img src={item.imageUrl} alt={item.name} className="block h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-xs font-semibold text-stone-500">Item</div>}</div>
                  <div><h3 className="text-xl leading-none tracking-[-0.03em]" style={{ color: 'var(--ink-strong)' }}>{item.name}</h3><p className="mt-2 text-sm" style={{ color: 'var(--ink-muted)' }}>{item.description || 'Produto pronto para checkout'}</p>{item.notes ? <p className="mt-2 text-sm leading-6" style={{ color: 'var(--ink-muted)' }}><strong style={{ color: 'var(--ink-strong)' }}>Obs.:</strong> {item.notes}</p> : null}{item.addons?.length ? <div className="mt-2 space-y-1 text-sm" style={{ color: 'var(--ink-muted)' }}>{item.addons.map((addon) => <p key={`${item.cartKey}-${addon.addon_id}`}>+ {addon.name} x{addon.quantity} ({formatMoney(addon.totalPrice)})</p>)}</div> : null}<p className="mt-3 text-sm font-semibold" style={{ color: 'var(--brand)' }}>{formatMoney(item.price)}</p></div>
                </div>
                <div className="flex flex-col items-end gap-3"><div className="flex items-center gap-2 rounded-full border px-2 py-1" style={{ borderColor: 'var(--line)' }}><button type="button" onClick={() => updateItemQuantity(item.cartKey, item.quantity - 1)} className="rounded-full p-1 hover:bg-black/5" style={{ color: 'var(--brand)' }}><Minus className="h-4 w-4" /></button><span className="min-w-6 text-center text-sm font-semibold">{item.quantity}</span><button type="button" onClick={() => updateItemQuantity(item.cartKey, item.quantity + 1)} className="rounded-full p-1 hover:bg-black/5" style={{ color: 'var(--brand)' }}><Plus className="h-4 w-4" /></button></div><p className="text-sm font-semibold" style={{ color: 'var(--ink-strong)' }}>{formatMoney(item.price * item.quantity)}</p></div>
              </div>
            </div>
          ))}
        </div>
...
function buildCartKey(productId: string, notes: string | null, addons: CartAddonSelection[]) {
  const normalizedAddons = [...addons].map((addon) => `${addon.addon_id}:${addon.quantity}`).sort().join('|');
  return `${productId}::${notes ?? ''}::${normalizedAddons}`;
}
