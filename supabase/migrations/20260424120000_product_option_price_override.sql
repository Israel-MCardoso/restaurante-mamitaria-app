...
set is_price_override = true
where lower(translate(trim(name), 'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇáàâãäéèêëíìîïóòôõöúùûüç', 'AAAAAEEEEIIIIOOOOOUUUUCaaaaaeeeeiiiiooooouuuuc')) like '%tamanho%';
  ...
  loop
    ...
    where po.product_id = product_row.id
      and (
        ...
        or lower(translate(trim(po.name), 'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇáàâãäéèêëíìîïóòôõöúùûüç', 'AAAAAEEEEIIIIOOOOOUUUUCaaaaaeeeeiiiiooooouuuuc')) like '%tamanho%'
      );
    ...
    if item_payload ? 'options' and jsonb_typeof(item_payload->'options') = 'array' then
      ...
      loop
        ...
        if coalesce(option_group_row.is_price_override, false)
          or lower(translate(trim(option_group_row.name), 'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇáàâãäéèêëíìîïóòôõöúùûüç', 'AAAAAEEEEIIIIOOOOOUUUUCaaaaaeeeeiiiiooooouuuuc')) like '%tamanho%' then
