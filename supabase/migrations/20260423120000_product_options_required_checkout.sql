...
drop policy if exists "Product option items are viewable by everyone" on public.product_option_items;
create policy "Product option items are viewable by everyone"
...
using (
  exists (
    ...
    where public.products.id = public.product_options.product_id
...
drop policy if exists "Admins can manage product option items" on public.product_option_items;
...
using (
  exists (
    ...
    join public.products
      on public.products.id = public.product_options.product_id
    join public.profiles
      on public.profiles.restaurant_id = public.products.restaurant_id
    where public.product_options.id = public.product_option_items.option_id
      and public.profiles.id = auth.uid()
      and public.profiles.role in ('admin', 'manager')
  )
)
      ...
      coalesce((
        select jsonb_agg(
          jsonb_build_object(
            ...
            'options', coalesce(oi.options_json, '[]'::jsonb)
          )
          order by oi.id
        )
...
begin
  ...
  loop
    ...
    item_unit_price := base_unit_price + addon_total_per_unit + option_total_per_unit;
